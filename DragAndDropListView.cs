...
namespace BioSeqDB
{
  public class DragAndDropListView : ListView
  {
    ...
    protected override void OnDragDrop(DragEventArgs drgevent)
    {
      ...
      ListViewItem hoverItem = GetItemAt(clientPoint.X, clientPoint.Y);
      ...
      else    // the user wishes to re-order the items
      {      
        int hoverIndex = hoverItem.Index;  // get the index of the hover item

        // determine if the items to be dropped are from this list view. If they are, perform a hack
        // to increment the hover index so that the items get moved properly.
        if (this == data.ListView)
        {
          if (hoverIndex > SelectedItems[0].Index)
            hoverIndex++;
        }
        ...
        {
          ListViewItem newItem = (ListViewItem)data.DragItems[i];
          Items.Insert(hoverIndex, newItem);
        }
      }
    ...
    protected override void OnDragOver(DragEventArgs drgevent)
    {
      ...
      if (Items.Count > 0)
      {
        ...
        if (hoverItem == null)
        {
          ...
          hoverItem = Items[Items.Count - 1];

          if (View == View.Details || View == View.List)
          {
            g.DrawLine(new Pen(m_lineColor, 2), new Point(hoverItem.Bounds.X, hoverItem.Bounds.Y + hoverItem.Bounds.Height), 
                                                  new Point(hoverItem.Bounds.X + Bounds.Width, hoverItem.Bounds.Y + hoverItem.Bounds.Height));
            g.FillPolygon(new SolidBrush(m_lineColor), new Point[] { new Point(hoverItem.Bounds.X, hoverItem.Bounds.Y + hoverItem.Bounds.Height - 5), 
            ...
            g.FillPolygon(new SolidBrush(m_lineColor), new Point[] { new Point(Bounds.Width - 4, hoverItem.Bounds.Y + hoverItem.Bounds.Height - 5), 
                                                  new Point(Bounds.Width - 9, hoverItem.Bounds.Y + hoverItem.Bounds.Height), new Point(Bounds.Width - 4, 
                                                          hoverItem.Bounds.Y + hoverItem.Bounds.Height + 5) });
          }
          else
          {
            g.DrawLine(new Pen(m_lineColor, 2), new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width, hoverItem.Bounds.Y), 
                                                 new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width, hoverItem.Bounds.Y + hoverItem.Bounds.Height));
            g.FillPolygon(new SolidBrush(m_lineColor), new Point[] { new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width - 5, hoverItem.Bounds.Y), 
                                                  new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width + 5, hoverItem.Bounds.Y), 
                                                  new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width, hoverItem.Bounds.Y + 5) });
            g.FillPolygon(new SolidBrush(m_lineColor), new Point[] { new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width - 5, hoverItem.Bounds.Y + hoverItem.Bounds.Height), 
                                                  new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width + 5, hoverItem.Bounds.Y + hoverItem.Bounds.Height), 
                                                  new Point(hoverItem.Bounds.X + hoverItem.Bounds.Width, hoverItem.Bounds.Y + hoverItem.Bounds.Height - 5) });
          }
        ...
        if (View == View.Details || View == View.List)
        {
          g.DrawLine(new Pen(m_lineColor, 2), new Point(hoverItem.Bounds.X, hoverItem.Bounds.Y), new Point(hoverItem.Bounds.X + Bounds.Width, hoverItem.Bounds.Y));
          ...
          g.FillPolygon(new SolidBrush(m_lineColor), new Point[] { new Point(Bounds.Width - 4, hoverItem.Bounds.Y - 5), new Point(Bounds.Width - 9, hoverItem.Bounds.Y), new Point(Bounds.Width - 4, hoverItem.Bounds.Y + 5) });
        }
        else
        {
          ...
          g.FillPolygon(new SolidBrush(m_lineColor), new Point[] { new Point(hoverItem.Bounds.X - 5, hoverItem.Bounds.Y + hoverItem.Bounds.Height),
                          new Point(hoverItem.Bounds.X + 5, hoverItem.Bounds.Y + hoverItem.Bounds.Height), 
                          new Point(hoverItem.Bounds.X, hoverItem.Bounds.Y + hoverItem.Bounds.Height - 5) });
        }

        // go through each of the selected items, and if any of the selected items have the same index as the item being hovered, disable dropping.
        foreach (ListViewItem itemToMove in SelectedItems)
        {
      ...
      drgevent.Effect = DragDropEffects.Move;   // everything is fine, allow the user to move the items
      base.OnDragOver(drgevent);   // call the base OnDragOver event
    }
    ...
    protected override void OnItemDrag(ItemDragEventArgs e)
    {
      ...
      DoDragDrop(GetDataForDragDrop(), DragDropEffects.Move);    // call the DoDragDrop method   
      base.OnItemDrag(e);   // call the base OnItemDrag event
    }
    ...
    protected override void OnLostFocus(EventArgs e)
    {
      ...
      base.OnLostFocus(e);   // call the OnLostFocus event
    }
    ...
    private DragItemData GetDataForDragDrop()
    {
      ...
      // go through each of the selected items and add them to the drag items collection by creating a clone of the list item
    ...
    {
      // determine if the previous item exists, if it does, reset the background and release the previous item
      if (m_previousItem != null)
      {
        m_previousItem = null;
      }
